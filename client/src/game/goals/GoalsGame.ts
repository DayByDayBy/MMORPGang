        ...
        for (const ball of this.balls) {
            ...
            for (let i = 0; i < this.players.length; i++) {
                ...
                if (saved) {
                    ...
                    // apply reflection to ball first
                    ball.velocity.x = ballState.vx;
                    ball.velocity.y = ballState.vy;

                    const goalDist = Math.sqrt(goalX * goalX + goalY * goalY) || 1;
                    const tangentX = -(goalY / goalDist);
                    const tangentY = (goalX / goalDist);
                    const tangentSpeed = player.paddleAngleVelocity * player.orbitRadius;

                    ball.addSpin(
                        { x: tangentX * tangentSpeed, y: tangentY * tangentSpeed },
                        0.6,
                        GOALS_MAX_BALL_SPEED,
                    );

                    ballState.vx = ball.velocity.x;
                    ballState.vy = ball.velocity.y;
