import { Graphics, Container } from "pixi.js";
import { BALL_RADIUS, BALL_SPEED, MAX_BALL_SPEED, reflectVelocity, clampSpeed } from "shared";
import type { Vector2 } from "shared";

export class Ball extends Container {
  public velocity: Vector2;
  public radius = BALL_RADIUS;
  private gfx = new Graphics();
  private serverX = 0;
  private serverY = 0;

  constructor() {
    super();
    this.velocity = { x: 0, y: 0 };
    this.addChild(this.gfx);
    this.draw();
  }

  private draw() {
    this.gfx.circle(0, 0, this.radius);
    this.gfx.fill({ color: 0xffffff });
    this.gfx.circle(0, 0, this.radius + 4);
    this.gfx.fill({ color: 0xffffff, alpha: 0.15 });
  }

  public update() {
    this.x += this.velocity.x;
    this.y += this.velocity.y;
  }

  public reflect(normal: Vector2) {
    const v = reflectVelocity(this.velocity.x, this.velocity.y, normal);
    this.velocity = clampSpeed(v.x, v.y, MAX_BALL_SPEED);
  }

  public addSpin(tangentVelocity: Vector2, influence = 0.6) {
    this.velocity.x += tangentVelocity.x * influence;
    this.velocity.y += tangentVelocity.y * influence;
    this.velocity = clampSpeed(this.velocity.x, this.velocity.y, MAX_BALL_SPEED);
  }

  public launch(target: Vector2) {
    this.x = 0;
    this.y = 0;
    const dist = Math.sqrt(target.x * target.x + target.y * target.y) || 1;
    this.velocity.x = (target.x / dist) * BALL_SPEED;
    this.velocity.y = (target.y / dist) * BALL_SPEED;
  }

  public syncState(serverBall: { x: number; y: number }) {
    this.serverX = serverBall.x;
    this.serverY = serverBall.y;
  }

  public interpolate() {
    this.x += (this.serverX - this.x) * 0.5;
    this.y += (this.serverY - this.y) * 0.5;
  }
}
